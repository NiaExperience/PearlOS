...
wizard_credentials() {
  ...
  echo ""
  echo "Choose TTS provider for bot:"
  echo "  1) Chorus TTS (local, open-source)"
  echo "  2) Pocket TTS (local HTTP service)"
  echo "  3) ElevenLabs (cloud, API key required)"
  read -r -p "Select [1-3] (default: 1): " tts_choice
  case "${tts_choice:-1}" in
    1)
      upsert_env_kv "$env_file" "BOT_TTS_PROVIDER" "kokoro"
      changed=1
      ;;
    2)
      upsert_env_kv "$env_file" "BOT_TTS_PROVIDER" "pocket"
      read -r -p "POCKET_TTS_URL (default: http://localhost:8766): " pocket_url
      upsert_env_kv "$env_file" "POCKET_TTS_URL" "${pocket_url:-http://localhost:8766}"
      changed=1
      ;;
    3)
      upsert_env_kv "$env_file" "BOT_TTS_PROVIDER" "elevenlabs"
      changed=1
  ...
  if $USE_TUI && is_interactive && ! $NON_INTERACTIVE; then
    ...
    if [[ "${SELECTED[11]}" == "1" ]]; then
      ...
      if [[ -n "$tui_result" ]]; then
        local env_file="${REPO_ROOT}/.env.local"
        if [[ -f "$env_file" ]]; then
          local daily_key openai_key deepgram_key elevenlabs_key tts_provider pocket_tts_url
          ...
          pocket_tts_url=$(echo "$tui_result" | grep -o '"POCKET_TTS_URL":"[^"]*"' | cut -d'"' -f4)
          [[ -z "$pocket_tts_url" ]] && pocket_tts_url=$(echo "$tui_result" | grep -o '"pocketTtsUrl":"[^"]*"' | cut -d'"' -f4)
          ...
          if [[ -n "$tts_provider" ]]; then
            # Bot expects BOT_TTS_PROVIDER. "chorus" maps to "kokoro".
            if [[ "$tts_provider" == "chorus" ]]; then
              upsert_env_kv "$env_file" "BOT_TTS_PROVIDER" "kokoro" && changed=1
            else
              upsert_env_kv "$env_file" "BOT_TTS_PROVIDER" "$tts_provider" && changed=1
            fi
          fi
          [[ -n "$pocket_tts_url" ]] && upsert_env_kv "$env_file" "POCKET_TTS_URL" "$pocket_tts_url" && changed=1
