...
def _task_workspace(task: dict) -> str:
    ...
    if workspace_mode == "operator_staging_source":
        source = os.path.abspath(os.getenv("PEARLOS_STAGING_SOURCE_ROOT", STAGING_SOURCE_ROOT))
        if requested_path and not _path_inside(requested_path, source):
            raise ValueError(f"refusing operator staging workspace outside source root: {requested_path}")
        return source
...
def _is_public_workspace_task(task: dict) -> bool:
    workspace_mode = str(task.get("workspace_mode") or "").strip()
    if workspace_mode in ("staging_source", "operator_staging_source", "internal_source"):
        return False
...
def _public_workspace_guard(task: dict) -> str:
    ...
    creation_launch_url = _creation_launch_url_from_workspace(workspace)
    studio_creation_line = (
        "Studio creation contract: this allowed workspace is the output folder for a user-scoped Studio app/game. "
        f"Create and verify index.html in this folder, keep supporting assets under this folder, "
        f"and make the final result include PLAY URL: {creation_launch_url}. "
        "Do not edit PearlOS source or send the user to a note as the primary deliverable.\n"
        if creation_launch_url
        else ""
    )
    return (
...
def _worker_cwd(task: dict) -> str:
    workspace_mode = str(task.get("workspace_mode") or "").strip()
    if workspace_mode in ("staging_source", "operator_staging_source") or _is_public_workspace_task(task):
...
When you finish:
- Return a concise user-facing final answer. The worker will write it back to
  the PearlOS task queue and may show it in chat, Notes, and task result views.
- Write 2-4 natural sentences that explain what the user can do now and whether
  it is live on the requested target.
- If something blocks you, explain the blocker and the next action needed in
  1-3 natural sentences.
- Do not format the final answer as "Changed", "Verification", "Not deployed",
  or a file-path checklist unless the user explicitly asked for that.
...
def _public_pearlos_app_base_url() -> str:
    raw = (
        ...
        or os.getenv("PEARL_INTERFACE_BASE_URL")
        or os.getenv("NEXT_PUBLIC_INTERFACE_URL")
...
def _is_auth_executor_error(text: str) -> bool:
    lowered = (text or "").lower()
    return any(
        needle in lowered
        for needle in (
...
def _is_sandbox_executor_error(text: str) -> bool:
    lowered = (text or "").lower()
    return any(
        needle in lowered
        for needle in (
...
def _plain_executor_error(err_msg: str, boss_name: str) -> tuple[str, str]:
    ...
    if _is_auth_executor_error(err_msg):
        return (
    ...
    if _is_sandbox_executor_error(err_msg):
        return (
            "the worker sandbox could not start cleanly",
            (
                " I stopped the automatic retries because another attempt would "
                "hit the same staging sandbox setup problem. The Codex sandbox "
                "smoke test needs to pass before this task is retried."
            ),
        )
...
def _public_task_failure_result(
    task: dict,
    *,
    boss_name: str,
    err_msg: str,
    ...
    title = _public_discord_content(_task_text_for_notice(task, 180), 180)
    if timed_out:
        reason = f"it ran for the full task time limit without finishing, so I stopped it"
    elif rc == 124:
        reason = "it timed out before finishing"
    elif rc < 0:
        reason = "the system stopped the worker process before it could finish"
    else:
        reason = "the worker process stopped before it reached a clean finish"
    err_snippet, next_step = _plain_executor_error(err_msg, boss_name)
    err_snippet = _public_discord_content(err_snippet, 220)
    blocker = f" The blocker was: {err_snippet}." if err_snippet else ""
    next_sentence = next_step.strip()
    message = f"I couldn't finish {title or 'that request'} because {reason}.{blocker}"
    if next_sentence:
        message = f"{message} {next_sentence}"
    return message.strip()


def _public_blocked_result(task: dict, boss_name: str, result: str) -> str:
    title = _public_discord_content(_task_text_for_notice(task, 180), 180)
    detail = _public_discord_content(result, 420)
    if not detail:
        detail = f"{boss_name} reported a blocker before producing a usable result."
    return (
        f"I couldn't finish {title or 'that request'} because the work hit a blocker. "
        f"{detail}"
    ).strip()


def _public_blocked_task_pattern_result(task: dict) -> str:
    title = _public_discord_content(_task_text_for_notice(task, 180), 180)
    return (
        f"I couldn't start {title or 'that request'} because it matches a staging safety block. "
        "I marked it failed without retrying so it will not stay pending."
    ).strip()


def _public_unactionable_task_result(task: dict) -> str:
    title = _public_discord_content(_task_text_for_notice(task, 180), 180)
    return (
        f"I couldn't start {title or 'that request'} because the task queue received operating rules "
        "without the actual request. Please send the concrete task again."
    ).strip()
...
class Worker:
    ...
    def _handle_completion(self, ift: InFlightTask, rc: int, timed_out: bool) -> None:
        ...
        if ift.executor in ("claude", "codex"):
            ...
            if rc != 0:
                ...
                public_failure = _public_task_failure_result(
                    task,
                    boss_name=boss_name,
                    err_msg=err_msg,
                    rc=rc,
                ...
                if stdout.strip():
                    ...
                    if false_success:
                        try:
                            self.api.fail(
                                tid,
                                _public_blocked_result(task, boss_name, stdout.strip()[:2000]),
            ...
            if rc == 0:
                ...
                try:
                    if _looks_like_blocked_result(result):
                        self.api.fail(
                            tid,
                            _public_blocked_result(task, "the worker", result),
                ...
                try:
                    self.api.fail(
                        tid,
                        _public_task_failure_result(
                            task,
                            boss_name="the worker",
                            err_msg=err,
                            rc=rc,
                            timed_out=rc == 124,
                        ),
                        note=f"executor exit {rc}",
                    )
    ...
    def _blocked_task_pattern_reason(self, task: dict) -> str:
        tid = task.get("id", "")
        ...
        for pattern in self.BLOCKED_TASK_PATTERNS:
            if pattern in title and pattern in body:
                reason = f"blocked task pattern {pattern!r} matched title and body"
                if tid not in self._seen_blocked:
                    self._seen_blocked.add(tid)
                    print(f"[BLOCKED] Skipping {tid}: pattern={pattern!r}  (matches both title+body)")
                return reason
            ...
            if pattern in body and pattern not in title:
                reason = f"blocked task pattern {pattern!r} matched body"
                if tid not in self._seen_blocked:
                    self._seen_blocked.add(tid)
                    print(f"[BLOCKED] Skipping {tid}: pattern={pattern!r}  (matches body only)")
                return reason

        return ""

    def _unactionable_task_reason(self, task: dict) -> str:
        title = (task.get("title") or "").lower()
        body = str(task.get("task") or "").strip()
        if not body:
            return "missing task body"
        ...
        meaningful_lines = [
            ...
            if not any(line.lower().startswith(prefix) for prefix in boilerplate_prefixes)
        ]
        meaningful_text = re.sub(r"\s+", " ", " ".join(meaningful_lines)).strip()
        vague_bodies = {"that", "this", "no", "yes", "continue", "keep going", "do it"}
        if meaningful_text.lower() in vague_bodies:
            return "task body has no concrete request"
        if "autonomous continuation obligation" in title and len(meaningful_text) < 80:
            return "autonomous continuation missing concrete request"
        return ""

    def _skip_decision(self, task: dict) -> tuple[bool, str, bool]:
        tid = task.get("id", "")
        # Pre-flight checks (cheap, run first)
        if task.get("status") != "pending":
            return True, "not pending", False
        if task.get("assignee") != self.assignee:
            return True, "different assignee", False
        if tid in self._in_flight:
            return True, "already in flight", False
        blocked_reason = self._blocked_task_pattern_reason(task)
        if blocked_reason:
            return True, blocked_reason, True
        unactionable_reason = self._unactionable_task_reason(task)
        if unactionable_reason:
            return True, unactionable_reason, True
        return False, "", False

    def _should_skip(self, task: dict) -> bool:
        skip, _, _ = self._skip_decision(task)
        return skip

    def _fail_blocked_pattern_task(self, task: dict, reason: str) -> bool:
        tid = task.get("id") or ""
        if not tid:
            return False
        result = (
            _public_blocked_task_pattern_result(task)
            if reason.startswith("blocked task pattern")
            else _public_unactionable_task_result(task)
        )
        note = (
            "worker refused blocked task pattern"
            if reason.startswith("blocked task pattern")
            else "worker refused unactionable task body"
        )
        try:
            ...
            if post_status not in ("failed", "completed", "cancelled", "archived"):
                log.warning("[BLOCKED] Refusal patch for %s came back non-terminal status=%s: %s", tid, post_status, reason)
                return False
            if post_status == "failed":
                log.warning("[BLOCKED] Marked %s failed/no_retry: %s", tid, reason)
            else:
                log.warning("[BLOCKED] Refusal patch for %s returned terminal status=%s: %s", tid, post_status, reason)
            self.completed += 1
            return True
        except Exception as e:
            log.error("[BLOCKED] Could not mark %s failed/no_retry: %s", tid, e)
            return False

    def _try_dispatch(self, task: dict) -> bool:
        """Claim and dispatch a task. Returns True if dispatched."""
        tid = task.get("id") or ""
        skip, skip_reason, terminal_skip = self._skip_decision(task)
        if skip:
            if terminal_skip:
                return self._fail_blocked_pattern_task(task, skip_reason)
            return False
