// Wonder Canvas runtime HTML — shared between the iframe renderer and the API route.
// Extracted to avoid duplication.

export const WONDER_RUNTIME_HTML = `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8"/>
...
/* ── Built-in animations ── */
.wonder-fadeIn{animation:wFadeIn .5s ease both}
.wonder-fadeOut{animation:wFadeOut .5s ease both}
.wonder-slideUp{animation:wSlideUp .5s cubic-bezier(.23,1,.32,1) both}
.wonder-slideDown{animation:wSlideDown .5s cubic-bezier(.23,1,.32,1) both}
.wonder-slideLeft{animation:wSlideLeft .5s cubic-bezier(.23,1,.32,1) both}
.wonder-slideRight{animation:wSlideRight .5s cubic-bezier(.23,1,.32,1) both}
.wonder-scaleIn{animation:wScaleIn .4s cubic-bezier(.23,1,.32,1) both}
.wonder-bounce{animation:wBounce .6s ease}
.wonder-shake{animation:wShake .5s ease}
.wonder-pulse{animation:wPulse 2s ease infinite}
.wonder-glow{animation:wGlow 3s ease infinite}
.wonder-float{animation:wFloat 4s ease-in-out infinite}
.wonder-spin{animation:wSpin 1s linear infinite}
.wonder-shimmer{animation:wShimmer 2.5s ease infinite}
...
@keyframes wSlideDown{from{opacity:0;transform:translateY(-28px)}to{opacity:1;transform:translateY(0)}}
@keyframes wSlideLeft{from{opacity:0;transform:translateX(28px)}to{opacity:1;transform:translateX(0)}}
@keyframes wSlideRight{from{opacity:0;transform:translateX(-28px)}to{opacity:1;transform:translateX(0)}}
@keyframes wScaleIn{from{opacity:0;transform:scale(0.9)}to{opacity:1;transform:scale(1)}}
...
@keyframes wPulse{0%,100%{transform:scale(1);opacity:1}50%{transform:scale(1.03);opacity:0.9}}
@keyframes wGlow{0%,100%{filter:brightness(1) drop-shadow(0 0 0px transparent)}50%{filter:brightness(1.15) drop-shadow(0 0 12px rgba(255,210,51,0.15))}}
...
.wonder-glass{background:rgba(255,255,255,0.04);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.08);border-radius:16px}
.wonder-glass-strong{background:rgba(255,255,255,0.08);backdrop-filter:blur(30px);-webkit-backdrop-filter:blur(30px);border:1px solid rgba(255,255,255,0.12);border-radius:16px}

/* ── Gradient line decoration ── */
.wonder-divider{width:40px;height:2px;background:linear-gradient(90deg,#FFD233,#E0547E);border-radius:2px}
.wonder-divider-wide{width:80px;height:1px;background:linear-gradient(90deg,transparent,rgba(155,109,255,0.4),transparent)}
...
@media (orientation:portrait),(max-width:600px){
  ...
  .wonder-layer:not(.wonder-app-managed) [style*="display:flex"][style*="flex-direction: row"],
  .wonder-layer:not(.wonder-app-managed) [style*="display: flex"][style*="flex-direction:row"],
  .wonder-layer:not(.wonder-app-managed) [style*="display:flex"]:not([style*="flex-direction:column"]):not([style*="flex-direction: column"]),
  .wonder-layer:not(.wonder-app-managed) [style*="display: flex"]:not([style*="flex-direction:column"]):not([style*="flex-direction: column"]){
    flex-direction:column !important;
  }
  .wonder-layer:not(.wonder-app-managed) [style*="display:flex"] > *,
  .wonder-layer:not(.wonder-app-managed) [style*="display: flex"] > *{
    width:100% !important;max-width:100% !important;
  }
  .wonder-layer:not(.wonder-app-managed) [style*="display:grid"][style*="grid-template-columns"],
  .wonder-layer:not(.wonder-app-managed) [style*="display: grid"][style*="grid-template-columns"]{
    grid-template-columns:1fr !important;grid-template-rows:auto auto !important;
  }
  .wonder-layer:not(.wonder-app-managed) .left,.wonder-layer:not(.wonder-app-managed) [class*="left"]{height:40vh;height:40dvh;min-height:200px}
  .wonder-layer:not(.wonder-app-managed) .right,.wonder-layer:not(.wonder-app-managed) [class*="right"]{flex:1;min-height:0;overflow-y:auto}
}
...
.wonder-choice{display:inline-block;padding:14px 28px;margin:6px;border-radius:14px;
  background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);color:#f0ece4;
  font-size:16px;font-family:Georgia,serif;letter-spacing:0.01em;
...
.wonder-choice:hover,.wonder-choice:active{background:rgba(255,210,51,.08);
  border-color:rgba(255,210,51,.25);transform:translateY(-2px);
  box-shadow:0 4px 20px rgba(0,0,0,0.3),0 0 20px rgba(255,210,51,0.06),inset 0 1px 0 rgba(255,255,255,0.1);
  color:#FFD233}
...
/* Portrait: stack class-named flex containers vertically.
   Uses :not(.wonder-app-managed) so apps that manage their own responsive layout
   can opt out by adding the wonder-app-managed class to the wonder-layer. */
body.portrait .wonder-layer:not(.wonder-app-managed) [class*="card"],
body.portrait .wonder-layer:not(.wonder-app-managed) [class*="container"],
body.portrait .wonder-layer:not(.wonder-app-managed) [class*="wrapper"],
body.portrait .wonder-layer:not(.wonder-app-managed) [class*="layout"],
body.portrait .wonder-layer:not(.wonder-app-managed) [class*="panel"],
body.portrait .wonder-layer:not(.wonder-app-managed) [class*="split"]{
  flex-direction:column !important;
}
/* Portrait: left/image panels become top ~40vh bands */
body.portrait .wonder-layer:not(.wonder-app-managed) [class*="left"],
body.portrait .wonder-layer:not(.wonder-app-managed) [class*="image"],
body.portrait .wonder-layer:not(.wonder-app-managed) [class*="hero"],
body.portrait .wonder-layer:not(.wonder-app-managed) [class*="media"],
body.portrait .wonder-layer:not(.wonder-app-managed) [class*="img"]{
...
body.portrait .wonder-layer:not(.wonder-app-managed) img{
...
body.portrait .wonder-layer:not(.wonder-app-managed) [class*="right"],
body.portrait .wonder-layer:not(.wonder-app-managed) [class*="text"],
body.portrait .wonder-layer:not(.wonder-app-managed) [class*="content"],
body.portrait .wonder-layer:not(.wonder-app-managed) [class*="info"],
...
(function(){
  ...
  function fixImages(container){
    container.querySelectorAll('img').forEach(function(img){
      ...
      img.decoding='async';
      img.loading='lazy';
      // Rewrite localhost:4444 URLs to relative paths (fixes mobile access)
      if(img.src && img.src.indexOf('localhost:4444')!==-1){
        img.src=img.src.replace(/https?:\\/\\/localhost:4444/g,'');
      }
      if(img.getAttribute('src') && img.getAttribute('src').indexOf('localhost:4444')!==-1){
        img.setAttribute('src',img.getAttribute('src').replace(/https?:\\/\\/localhost:4444/g,''));
      }
      // Fix relative URLs that break in srcdoc iframes (resolve against parent origin)
      var rawSrc=img.getAttribute('src');
      if(rawSrc && rawSrc.charAt(0)==='/' && rawSrc.charAt(1)!=='/'){
        try{img.src=window.location.origin+rawSrc}catch(e){}
      }
      img.onerror=function(){
        console.warn('[wonder] Image failed to load:',img.src);
        // Show styled placeholder instead of browser broken-image icon / question mark
        img.style.display='inline-block';
        img.style.minWidth='60px';
        img.style.minHeight='40px';
        img.style.background='rgba(255,255,255,0.05)';
        img.style.border='1px dashed rgba(255,255,255,0.2)';
  ...
  function execScripts(container){
    // Scripts injected via innerHTML don't execute per spec.
    // Clone each <script> into a new element so the browser runs it.
  ...
  function setScene(data){
    ...
    var isAppManaged=html.indexOf('wonder-app-managed')!==-1;
    layer.classList.toggle('wonder-app-managed',isAppManaged);
  ...
  function clearAll(data){
    if(data&&data.layer){
      ...
      Object.keys(layers).forEach(function(k){layers[k].classList.remove('wonder-app-managed');layers[k].innerHTML=''});
    }
