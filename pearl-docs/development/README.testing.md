...
- **Fixtures** and **support files** are in the `tests/cypress/` directory.
...
- **Cypress** is used for E2E tests in the interface app.
  - Configured via `tests/cypress.config.js` and `tests/cypress/` directory.
...
For any questions or to contribute to the testing strategy, see `jest.config.mjs`, `tests/cypress.config.js`, and the `packages/prism/__tests__` directory for examples.
