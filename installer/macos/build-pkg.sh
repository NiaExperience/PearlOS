...
if [[ -z "${VERSION}" || -z "${PAYLOAD_ROOT}" ]]; then
  echo "Usage: build-pkg.sh --version <v> --payload-root <dir>" >&2
  exit 2
fi
...
# - The real macOS .pkg is implemented in the `macos-installer` task.
OUT_TGZ="${DIST_DIR}/PearlOS_Payload_macOS_${VERSION}.tar.gz"
rm -f "${OUT_TGZ}"
